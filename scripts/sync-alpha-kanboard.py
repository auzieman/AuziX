...
parser.add_argument('--state', choices=['working', 'validating', 'blocked'])
...
parser.add_argument('--block-ax001', action='store_true', help='Mark failed task54 validation Blocked')
args = parser.parse_args()
if args.state and not args.issue:
    parser.error('--state requires --issue')
...
if sys.argv[2].startswith('state:'):
    assert len(items)==1
    key=items[0]['reference'].split(':')[-1]
    tid=53+int(key.split('-')[-1])
    task=rpc('getTask',{'task_id':tid})
    assert 'ai_worker_ref:'+items[0]['reference'] in task['description']
    column={'working':11,'validating':35,'blocked':41}[sys.argv[2].split(':')[1]]
    if int(task['column_id'])!=column or int(task['swimlane_id'])!=3:
        assert rpc('moveTaskPosition',{'project_id':3,'task_id':tid,'column_id':column,'position':1,'swimlane_id':3})
    check=rpc('getTask',{'task_id':tid})
    assert int(check['column_id'])==column and int(check['swimlane_id'])==3
    print(json.dumps({'status':'verified','task_id':tid,'column_id':column}))
    raise SystemExit()
if sys.argv[2] in ('start-ax001','validate-ax001','block-ax001'):
    task=rpc('getTask',{'task_id':54})
    assert 'ai_worker_ref:auzix-alpha-20260905:AX-001' in task['description']
    column={'validate-ax001':35, 'start-ax001':11, 'block-ax001':41}[sys.argv[2]]
    if int(task['column_id']) != column or int(task['swimlane_id']) != 3:
        rpc('moveTaskPosition',{'project_id':3,'task_id':54,'column_id':column,'position':1,'swimlane_id':3})
    task=rpc('getTask',{'task_id':54})
...
mode = 'block-ax001' if args.block_ax001 else ('validate-ax001' if args.validate_ax001 else ('start-ax001' if args.start_ax001 else ('order' if args.order_columns else 'sync')))
if args.state:
    mode='state:'+args.state
...
if args.state or args.order_columns or args.start_ax001 or args.validate_ax001 or args.block_ax001:
